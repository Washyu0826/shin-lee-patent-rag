...
- 直接事實：「列出 I918613 號專利的 IPC 分類」
- 間接語意：「有沒有跟能量回收相關的專利？」
- 跨文比較：「比較 TW202401234A 與 TW202405678B 的技術差異」
- 專有名詞：「偏光膜相關的專利有哪些？」
...
**重點觀察**：間接語意題從 0.0014 → 0.889，**提升 635 倍**。這就是 vocabulary gap 問題 — MiniLM 抓不到「能量回收」與「再生煞車」的語意關聯，bge-m3 sparse 加 HyDE 擴展後完全解決。
...
- Ollama（裝在本機或遠端）
- Qdrant（建議 Docker 跑）
- PostgreSQL（**選用** — 沒裝會 fallback 到 SQLite）
...
| `DATABASE_URL` | `postgresql://...` | 沒設或連不到自動 fallback SQLite |
...
CI 在每個 push 自動跑 22 個 test（pytest, ubuntu-latest, ~28s）。
